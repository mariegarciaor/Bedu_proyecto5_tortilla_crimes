...
#no funcionan
tortilla_prices$state <- gsub("Edo. México", "Estado de Mexico", tortilla_prices$state)
tortilla_prices$state <- gsub("Nuevo León", "Nuevo Leon", tortilla_prices$state)
tortilla_prices$state <- gsub("San Luis Potosí", "San Luis Potosi", tortilla_prices$state)
...
unique(mexico_crime$year)
unique(tortilla_prices$year)


### 
sample_data <- tortilla_prices %>% sample_n(10)
